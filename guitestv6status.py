...
def login():
    ...
    entry11 = Entry(login_window, show = '*', textvariable = password_verify)
...
def logincheck():
    ...
    if cur.count()==0:
        ...
        if curpass.count()==0:
            ...
            print('Login Information Verified.')
...
def studentinfo():
    ...
    studentreadout = StringVar()
    studentreadout = col.find({'status' : ident})

    student_window = tk.Toplevel()
    student_window.geometry('1000x300')
    ...
    label1V = Label(student_window, textvariable = studentreadout).pack()
    label2VVV = Label(student_window, text = ('')).pack()
    buttonstatusin = Button(student_window, text='Check In', height='2', width="30", command = checkin).pack()
    label2VV = Label(student_window, text = ('')).pack()
    buttonstatusout = Button(student_window, text='Check Out', height='2', width="30", command = checkout).pack()
    label2V = Label(student_window, text = ('')).pack()
    button2V = Button(student_window, text='Quit to Login', height='2', width="30", command = quitinfo).pack()
    label3V = Label(student_window, text = ('')).pack()
    button3V = Button(student_window, text='Quit Program', height='2', width="30", command = quitfunction).pack()
